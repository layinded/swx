...
class AnthropicProvider(BaseLLMProvider):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            return LLMResponse(False, "", error=str(exc), model=model_name, provider="anthropic", latency_ms=int((monotonic() - started) * 1000))

    async def stream(self, request: LLMRequest) -> AsyncGenerator[str, None]:
        model_name = self.config.get("model_name", "claude-3-5-sonnet-latest")
        async with self.client.messages.stream(model=model_name, system=_system_prompt(request) or None, messages=[{"role": "user", "content": request.prompt}], temperature=request.temperature, max_tokens=request.max_tokens, top_p=request.top_p) as stream:
