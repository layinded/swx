...
async def create_organization(session: AsyncSession, data: OrganizationCreate, owner_id: UUID) -> OrganizationPublic:
    ...
    await organization_repository.add_member(session, {"organization_id": organization.id, "user_id": owner_id, "role": OrganizationRole.OWNER.value, "invited_by": owner_id, "joined_at": utc_now_naive(), "is_active": True})
...
async def accept_invitation(session: AsyncSession, token: str, user_id: UUID) -> OrganizationMemberPublic:
    ...
    if not member:
        member = await organization_repository.add_member(session, {"organization_id": invitation.organization_id, "user_id": user_id, "role": invitation.role, "invited_by": invitation.inviter_id, "joined_at": utc_now_naive(), "is_active": True})
    public_member = OrganizationMemberPublic.model_validate(member)
    await organization_repository.update_invitation_status(session, invitation.id, InvitationStatus.ACCEPTED.value, accepted_at=utc_now_naive())
...
async def reject_invitation(session: AsyncSession, token: str) -> OrganizationInvitationPublic:
    ...
    if not invitation:
        raise HTTPException(status_code=404, detail="Invitation not found")
    updated = await organization_repository.update_invitation_status(session, invitation.id, InvitationStatus.REJECTED.value, rejected_at=utc_now_naive())
    if not updated:
        raise HTTPException(status_code=404, detail="Invitation not found")
