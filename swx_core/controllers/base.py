...
class BaseController(Generic[ModelType, CreateSchema, UpdateSchema, PublicSchema]):
    ...
    async def restore(
        ...
        item = await self.service.update(
            id, {"is_deleted": False}, emit_event=emit_event
        )
    ...
    async def bulk_create(
        ...
        await self.service.event_bus.emit(
            f"{self.model.__name__.lower()}.bulk_created", {"count": len(items)}
        )
    ...
    async def bulk_update(
        ...
        await self.service.event_bus.emit(
            f"{self.model.__name__.lower()}.bulk_updated", {"count": len(items)}
        )
    ...
    async def bulk_delete(
        ...
        await self.service.event_bus.emit(
            f"{self.model.__name__.lower()}.bulk_deleted", {"count": count}
        )
    ...
    def register_crud_routes(
        ...
        if include_create and self.schema_create:
            ...
            @self.router.post(
                "",
                response_model=self.schema_public,
        ...
        if include_search:
            ...
            @self.router.get(
                "/search", response_model=PaginatedResponse[self.schema_public]
            )
