...
class WalletAdjustmentRequest(Base, table=True):
    ...
    proposed_by: uuid.UUID = Field(sa_column=Column(PG_UUID(as_uuid=True), ForeignKey("swx_admin_users.id"), nullable=False))
    approved_by: Optional[uuid.UUID] = Field(default=None, sa_column=Column(PG_UUID(as_uuid=True), ForeignKey("swx_admin_users.id"), nullable=True))
