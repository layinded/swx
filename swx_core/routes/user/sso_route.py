...
from ...auth.user.dependencies import UserDep
...
async def initiate_sso(body: SSOInitiateRequest, session: AsyncSession = Depends(get_session), user: User = Depends(UserDep)):
...
async def list_enabled_providers(skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=200), session: AsyncSession = Depends(get_session), _user: User = Depends(UserDep)):
...
async def list_my_sessions(skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=200), session: AsyncSession = Depends(get_session), user: User = Depends(UserDep)):
...
async def terminate_my_session(sso_session_id: UUID, session: AsyncSession = Depends(get_session), user: User = Depends(UserDep)):
