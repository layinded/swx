...
async def list_filters(
    ...
    _admin: object = Depends(get_current_admin_user),
):
...
async def create_filter(body: ContentFilterCreate, session: AsyncSession = Depends(get_session), _admin: object = Depends(get_current_admin_user)):
    return await create_filter_controller(session, body)


@router.get("/filters/{filter_id}", response_model=ContentFilterPublic)
async def get_filter(filter_id: UUID, session: AsyncSession = Depends(get_session), _admin: object = Depends(get_current_admin_user)):
    return await get_filter_controller(session, filter_id)


@router.put("/filters/{filter_id}", response_model=ContentFilterPublic)
async def update_filter(filter_id: UUID, body: ContentFilterUpdate, session: AsyncSession = Depends(get_session), _admin: object = Depends(get_current_admin_user)):
...
async def delete_filter(filter_id: UUID, session: AsyncSession = Depends(get_session), _admin: object = Depends(get_current_admin_user)):
...
async def get_check(check_id: UUID, session: AsyncSession = Depends(get_session), _admin: object = Depends(get_current_admin_user)):
