...
from ...auth.admin.dependencies import get_current_admin_user
...
router = APIRouter(prefix="/sso", tags=["Admin - SSO"])


@router.get("/providers", response_model=list[SSOProviderPublic])
async def list_providers(skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=200), session: AsyncSession = Depends(get_session), _admin: dict = Depends(get_current_admin_user)):
...
async def create_provider(body: SSOProviderCreate, session: AsyncSession = Depends(get_session), _admin: dict = Depends(get_current_admin_user)):
    return await create_provider_controller(session, body)


@router.get("/providers/{provider_id}", response_model=SSOProviderPublic)
async def get_provider(provider_id: UUID, session: AsyncSession = Depends(get_session), _admin: dict = Depends(get_current_admin_user)):
...
async def update_provider(provider_id: UUID, body: SSOProviderUpdate, session: AsyncSession = Depends(get_session), _admin: dict = Depends(get_current_admin_user)):
...
async def delete_provider(provider_id: UUID, session: AsyncSession = Depends(get_session), _admin: dict = Depends(get_current_admin_user)):
    return await delete_provider_controller(session, provider_id)


@router.get("/sessions", response_model=list[SSOSessionPublic])
async def list_sessions(skip: int = Query(default=0, ge=0), limit: int = Query(default=100, ge=1, le=200), session: AsyncSession = Depends(get_session), _admin: dict = Depends(get_current_admin_user)):
...
async def terminate_session(sso_session_id: UUID, session: AsyncSession = Depends(get_session), _admin: dict = Depends(get_current_admin_user)):
