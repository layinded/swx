...
def doctor_command(fix: bool, verbose: bool, json_output: bool):
    ...
    for check, status in dep_checks.items():
        if status["status"] == "ok":
            ...
            click.secho(
                f"   ❌ {check}: {status.get('message', 'not installed')}", fg="red"
            )
            results["errors"].append(
                f"{check}: {status.get('message', 'not installed')}"
            )
    ...
    if results["status"] == "healthy":
        ...
        click.secho(
            f"⚠️  System is degraded ({len(results['warnings'])} warnings)",
            fg="yellow",
            bold=True,
        )
    else:
        click.secho(
            f"❌ System is unhealthy ({len(results['errors'])} errors, {len(results['warnings'])} warnings)",
            fg="red",
            bold=True,
        )
...
def _check_environment() -> Dict[str, Dict[str, Any]]:
        ...
        if secret_key:
            if len(secret_key) < 32:
                checks["SECRET_KEY strength"] = {
                    "status": "warning",
                    "message": f"{len(secret_key)} chars (need 32+)",
                }
            else:
    ...
    if sys.version_info >= (3, 10):
        ...
        checks["Python version"] = {
            "status": "error",
            "message": f"{py_version} (need 3.10+)",
        }
...
def _check_security(verbose: bool, fix: bool) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        if len(secret) < 32:
            checks["SECRET_KEY length"] = {
                "status": "warning",
                "message": f"{len(secret)} chars (need 32+)",
            }
        else:
        ...
        if any(p in secret.lower() for p in weak_patterns):
            checks["SECRET_KEY entropy"] = {
                "status": "warning",
                "message": "appears to contain weak patterns",
            }
        else:
    ...
    try:
        ...
        if env == "production":
            ...
            if "dev" in secret.lower() or "test" in secret.lower():
                checks["Production secret"] = {
                    "status": "error",
                    "message": "using dev/test secret in production",
                }
            else:
