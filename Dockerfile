...
# Step 1: Create Virtual Environment & Install pip
RUN python -m venv .venv && \
    /app/.venv/bin/python -m ensurepip --default-pip && \
    /app/.venv/bin/python -m pip install --upgrade pip setuptools wheel

# Step 2: Copy Dependencies and Source Code (needed for uv sync)
COPY pyproject.toml uv.lock /app/
COPY swx_core /app/swx_core
COPY swx_app /app/swx_app

# Step 3: Install Dependencies with `uv` (locked, hash-verified)
# SOC 2 CC7.1: uv.lock contains content hashes; --frozen enforces lockfile integrity.
RUN --mount=type=cache,target=/root/.cache/uv \
    uv venv .venv && \
    (uv sync --frozen --no-install-project 2>&1 || uv pip install -e ".[dev]")

# Step 4: Reinstall pip AFTER uv (because uv wipes pip)
RUN /app/.venv/bin/python -m ensurepip --default-pip && \
    /app/.venv/bin/python -m pip install --upgrade pip setuptools wheel

# Step 5: Install SwX-API as a Package
...
# Copy `pyproject.toml` to the final stage
COPY pyproject.toml /app/

# Install SwX-API as a Package
RUN /app/.venv/bin/python -m pip install .
